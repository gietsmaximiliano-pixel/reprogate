...
function isReceipt(value: unknown): value is ReproGateReceipt {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value as Partial<ReproGateReceipt>;
  return (
    candidate.receiptFormatVersion === "0.1" &&
...
async function collectJsonFiles(inputDir: string): Promise<string[]> {
...
export async function loadReceipts(inputDir: string): Promise<ReproGateReceipt[]> {
  const files = await collectJsonFiles(inputDir);
  const receipts: ReproGateReceipt[] = [];
...
export function summarizeReceipts(receipts: ReproGateReceipt[]): DashboardStats {
  const reportTypes: Partial<Record<ReportType, number>> = {};
  let valid = 0;
  let scoreSum = 0;

  for (const receipt of receipts) {
    if (receipt.validation.valid) valid += 1;
...
function renderBreakdown(stats: DashboardStats): string {
  const entries = Object.entries(stats.reportTypes).sort(([a], [b]) => a.localeCompare(b));
  if (entries.length === 0) return "<p>No report types recorded yet.</p>";
