...
export const RECEIPT_FORMAT_VERSION = "0.1";
...
export const evidenceKinds = ["fixture", "log", "screenshot", "trace", "other"];
...
export function validateManifest(input) {
    ...
    if (!parsed.success) {
        ...
        for (const issue of parsed.error.issues) {
            ...
            const isMissing = issue.code === "invalid_type" && "received" in issue && issue.received === "undefined";
            if (isMissing) {
...
export function parseManifestText(text, sourcePath = "reprogate.yml") {
...
export function resolveEvidencePath(baseDir, evidencePath) {
...
export async function verifyEvidenceFiles(manifest, baseDir) {
...
export function calculateActionabilityScore(manifest, validation) {
    if (!manifest) {
        return {
            score: 0,
    ...
    if (hasText(manifest.environment.os))
        add(6, "Operating system is described.");
    if (hasText(manifest.environment.runtime))
        add(5, "Runtime information is included.");
    if (manifest.environment.dependencies &&
    ...
    if (hasList(manifest.commands))
        add(8, "Reporter supplied reproduction commands.");
    const evidenceCount = (manifest.evidenceFiles?.length ?? 0) +
    ...
    if (evidenceCount > 0)
        add(8, "Evidence references are included.");
    ...
    if (hasText(manifest.redactionNotes))
        add(4, "Redaction notes are provided.");
    ...
    if (validation && !validation.valid) {
        const penalty = Math.min(40, validation.missingFields.length * 8 + validation.invalidFields.length * 5);
        if (penalty > 0) {
            score -= penalty;
            reasons.push(`-${penalty}: Schema validation issues reduce actionability.`);
        }
    }
    const failedEvidence = validation?.evidence.filter((item) => !item.exists || !item.matches).length ?? 0;
    if (failedEvidence > 0) {
...
export async function validateManifestWithEvidence(input, baseDir) {
...
export function renderMarkdownSummary(manifest, score) {
    const lines = [
    ...
    lines.push("## Environment", "", `- **OS:** ${manifest.environment.os}`, `- **Runtime:** ${manifest.environment.runtime ?? "Not provided"}`);
    if (manifest.environment.dependencies &&
        Object.keys(manifest.environment.dependencies).length > 0) {
    ...
    lines.push("", "## Expected Behavior", "", manifest.expectedBehavior, "", "## Actual Behavior", "", manifest.actualBehavior, "", "## Steps to Reproduce", "");
