...
import { calculateActionabilityScore, createReceipt, normalizeManifest, readManifestFile, renderMarkdownSummary, validateManifest, validateManifestWithEvidence } from "@reprogate/spec";
...
export async function initProject(rootDir, options = {}) {
    const files = [
...
export async function loadManifestWithPath(filePath) {
...
function printValidationResult(filePath, validation) {
    ...
    console.log("\nScore reasons:");
    for (const reason of actionability.reasons)
        console.log(`- ${reason}`);
}
async function createManifestInteractively() {
    ...
    if (!validation.valid) {
        throw new Error(`Generated manifest was invalid: ${validation.invalidFields.map((item) => item.message).join(", ")}`);
    }
...
async function runSafeVerification(filePath, options) {
...
export function buildProgram() {
    ...
    program
        ...
        const markdown = renderMarkdownSummary(manifest, calculateActionabilityScore(manifest, validation));
        if (options.output) {
    ...
    program
        ...
        .action(async (receiptsDir, options) => {
        const result = await generateDashboard(receiptsDir, options.output);
        console.log(`Dashboard written to ${result.outputPath}`);
        console.log(`Receipts: ${result.stats.total}, valid: ${result.stats.valid}, invalid: ${result.stats.invalid}`);
    });
    return program;
}
async function main() {
